...
const LoginPopup: React.FC<LoginPopupProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Note: The popup will close automatically after successful Google sign-in
      // due to the redirect flow
    } catch (error: any) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px] p-4 sm:p-6">
        <DialogHeader>
          <DialogTitle className="text-xl sm:text-2xl">Welcome to JETA</DialogTitle>
        </DialogHeader>
